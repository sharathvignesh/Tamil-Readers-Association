 ...
 *
 * This file compresses the TinyMCE CSS using GZip and
 ...
 */
        ...
        // Get input
        $custom = explode(',', getParam("f", ""));
        $cachePath = realpath("."); // Cache path, this is where the .gz files will be stored
        $expiresOffset = 3600 * 24 * 5; // Cache for 10 days in browser cache
        $content = "";
        $encodings = array();
        $supportsGzip = false;
        $enc = "";
        ...
        $cacheFile = $cachePath . "/compress_" . $cacheKey . ".gz";
        ...
        if ($supportsGzip && file_exists($cacheFile)) {
                ...
                // Add custom files
                foreach ($custom as $file)
                        $content .= getFileContents($file.'.css');
